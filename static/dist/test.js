...
const maxQualityButtonDefaults = {
	parent: null
};
...
class MaxQualityButton extends MenuButton {
	...
	constructor(player, options) {
		...
		this.options = videojs.mergeOptions(maxQualityButtonDefaults, options);
 ...
 */
class MaxQualitySelector extends videojs.getPlugin('plugin') {
	/**
		...
		if (this.player.qualityLevels !== undefined) {
			...
			const buttonIndex =
				this.options.index < 0
					? player.controlBar.children().length + this.options.index
					: this.options.index;
	...
	changeLevel(levelIndex) {
		...
		if (levelIndex < 0) {
			...
			this.qlInternal.levels_.forEach(function (obj, idx) {
				if (
					(self.options.minHeight !== 0 && obj.height < self.options.minHeight) ||
					(self.options.maxHeight !== 0 && obj.height > self.options.maxHeight)
				) {
					obj.enabled = false;
				} else {
					obj.enabled = true;
	...
	handleQualityLevel(e) {
		...
		if (
			(this.options.minHeight !== 0 && ql.height < this.options.minHeight) ||
			(this.options.maxHeight !== 0 && ql.height > this.options.maxHeight)
		) {
			ql.enabled = false;
			return;
		}
	...
	getQualityDisplayString(qualityLevel) {
		...
		if (this.options.displayMode === 1) {
			...
			displayString =
				qualityLevel.dimensionMarketingName +
				'<sup>' +
				qualityLevel.dimensionEnglishName +
				'</sup>';
		}
